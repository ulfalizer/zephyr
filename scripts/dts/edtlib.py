...
class Device:
    ...
    def _init_interrupts(self):
        ...
        self.interrupts_tmp = _mapped_interrupts(_unmapped_interrupts(self._node))
...
def _unmapped_interrupts(node):
    # _init_interrupts() helper. Returns returns a list of
    # (<interrupt-parent>, <raw interrupt specifier>) tuples for the interrupts
    # generated by 'node', before any 'interrupt-map' has been applied.
    ...
    if "interrupts-extended" in node.props:
        ...
        while raw:
            ...
            res.append((iparent, raw[4*interrupt_cells:]))
...
def _mapped_interrupts(interrupts):
    # TODO: merge logic into _unmapped_interrupts() helper function and rename

    res = []

    for iparent, raw_interrupt in interrupts:
        interrupt_cells = _interrupt_cells(iparent)

        if "interrupt-controller" in iparent.props:
            # No mapping
            res.append((iparent, [to_num(raw_interrupt[4*i:4*i + 4])
                                  for i in range(interrupt_cells)]))
        elif "interrupt-map" in iparent.props:
            # TODO
            pass
